    ...
    def fit(
        ...
        **kw,
        ...
        for epoch in range(num_epochs):
            ...
            train_loss += torch.FloatTensor(train_result.losses).tolist()
            train_acc.append(train_result.accuracy)
            # test_loss.append(torch.mean(torch.FloatTensor(test_result.losses)).item())
            test_loss += torch.FloatTensor(test_result.losses).tolist()
...
class TransformerEncoderTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        # ====== YOUR CODE: ======
        self.optimizer.zero_grad()
        predictions = self.model(input_ids, attention_mask).to(self.device)
        loss = self.loss_fn(predictions, label)
        loss.backward()
        self.optimizer.step()
        sig = torch.sigmoid(predictions)
        num_correct = (torch.round(sig) == label).sum()
        # ========================
...
class FineTuningTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        model_fine = self.model(input_ids=input_ids, labels=labels,attention_mask=attention_masks)
        #raw scores produced by model before activation
        logits = model_fine.logits
        loss = model_fine.loss
        ...
        loss = loss.item()
        num_correct = (torch.argmax(torch.sigmoid(logits), dim=1) == labels).sum().item()
