...
class EncoderCNN(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        # ====== YOUR CODE: ======
        modules = [
            nn.Conv2d(in_channels, 64, kernel_size=4, stride=2, padding=1),  # (N, 64, H/2, W/2)
            ...
            nn.ReLU(),
            # nn.Dropout(0.1),
            nn.Conv2d(256, out_channels, kernel_size=4, stride=2, padding=1),  # (N, out_channels, H/16, W/16)
...
class DecoderCNN(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        # ====== YOUR CODE: ======
        modules = [
            nn.ConvTranspose2d(in_channels, 256, kernel_size=4, stride=2, padding=1),  # (N, 256, H/8, W/8)
            nn.BatchNorm2d(256, eps=1e-5, momentum=0.1,affine = True),
            nn.ReLU(),
            # nn.Dropout(0.1),
            nn.ConvTranspose2d(256, 128, kernel_size=4, stride=2, padding=1),  # (N, 128, H/4, W/4)
            nn.BatchNorm2d(128, eps=1e-5, momentum=0.1,affine = True),
            nn.ReLU(),
            # nn.Dropout(0.1),
            nn.ConvTranspose2d(128, 64, kernel_size=4, stride=2, padding=1),  # (N, 64, H/2, W/2)
            nn.BatchNorm2d(64, eps=1e-5, momentum=0.1,affine = True),
            nn.ReLU(),
            # nn.Dropout(0.1),
            nn.ConvTranspose2d(64, out_channels, kernel_size=4, stride=2, padding=1),  # (N, out_channels, H, W)
...
class VAE(nn.Module):
    def __init__(self, features_encoder, features_decoder, in_size, z_dim):
        ...
        # ====== YOUR CODE: ======
        # Define fully connected layers for mu and log_sigma2
        self.fc_mu = nn.Linear(n_features, z_dim)
        self.fc_log_sigma2 = nn.Linear(n_features, z_dim)
        # Define fully connected layer to convert z to features h
        self.fc_z_to_h = nn.Linear(z_dim, n_features)
    ...
    def encode(self, x):
        ...
        # ====== YOUR CODE: ======
        # Produce a feature vector h from the input image x
        h = self.features_encoder(x)
        h = h.view(h.size(0), -1)
    ...
    def sample(self, n):
        ...
        with torch.no_grad():
            ...
            for _ in range(n):
                ...
                samples.append(sample.squeeze(0))  # Remove the batch dimension
...
def vae_loss(x, xr, z_mu, z_log_sigma2, x_sigma2):
    ...
    norm_vec = torch.norm((x - xr).reshape(x.shape[0], -1), dim=1)
    data_loss = torch.mean(norm_vec ** 2) / (dx * x_sigma2)
    kldiv_loss = torch.mean(torch.exp(z_log_sigma2).sum(1) + torch.norm(z_mu, dim=1) ** 2 - z_mu.shape[1] - z_log_sigma2.sum(1))
