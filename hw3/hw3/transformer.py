...
def sliding_window_attention(q, k, v, window_size, padding_mask=None):
    ...
    #setup query and key vectors for diagonal windows
    Q_Vectors = []
    Key_Vectors = []

    ##half window before diagonal:
    for i in range(window_size // 2):
        Q_Vectors.append(list(i for _ in range(window_size // 2 + 1 + i)))
        Key_Vectors.append(list(j for j in range(window_size // 2 + 1 + i)))
    #diagonal windows:
    for i in range(window_size // 2, seq_len - window_size // 2):
        Q_Vectors.append(list(i for _ in range(window_size + 1)))
        Key_Vectors.append(list(j for j in range(i - window_size // 2, i + window_size // 2 + 1)))
    ##half window after diagonal:
    for i in (range(window_size // 2)):
        Q_Vectors.append(list((seq_len - window_size // 2 + i) for _ in range(window_size - i)))
        Key_Vectors.append(list((seq_len - window_size + j + i) for j in range(window_size - i)))
    #flatten the lists so einsum knows which indexes to multiply:
    Q_Vectors = np.concatenate(Q_Vectors)
    Key_Vectors = np.concatenate(Key_Vectors)

    #compute attention:
    if q.dim() > 3:
        attention = torch.full((batch_size, q.shape[1], seq_len, seq_len), -9e15).to(q.device)
    else:
        attention = torch.full((batch_size, seq_len, seq_len), -9e15).to(q.device)
    attention[:, ..., Q_Vectors, Key_Vectors] = torch.einsum(
        '...ij,...jk->...ik',
        q[:, ..., Q_Vectors, :].unsqueeze(-2),
        k[:, ..., Key_Vectors, :].unsqueeze(-1)
    ).squeeze(-1).squeeze(-1)
    ...
    if padding_mask is not None: 
        ...
        #fill mask with -inf:
        attention.masked_fill_(mask=(padding_mask==0), value=torch.tensor(-9e15))
    
    #normalize + softmax attention:
...
class Encoder(nn.Module):
    ...
    def forward(self, sentence, padding_mask):
        ...
        # ====== YOUR CODE: ======
        embedded_sequence = self.encoder_embedding(sentence)

        encoded_sequence = self.dropout(self.positional_encoding(embedded_sequence))

        encoded_sequence = torch.cat([layer(encoded_sequence, padding_mask) for layer in self.encoder_layers], dim=1)

        output = self.classification_mlp(encoded_sequence[:, 0]).squeeze(-1)
