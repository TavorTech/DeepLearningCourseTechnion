...
def create_train_validation_loaders(
    ...
    #  you create.
    # ====== YOUR CODE: ======
    size_with_ratio = int(len(dataset) * validation_ratio)
    indices = torch.randperm(len(dataset))
    train_sample = torch.utils.data.SubsetRandomSampler(indices[size_with_ratio :])
    is_valid_sample = torch.utils.data.SubsetRandomSampler(indices[:size_with_ratio])
    ...
    dataload_valid =  torch.utils.data.DataLoader(
        dataset=dataset,
        sampler= is_valid_sample,
        batch_size=batch_size,
        num_workers=2)
    
    #raise NotImplementedError()
    # ========================
    return dataload_train, dataload_valid
