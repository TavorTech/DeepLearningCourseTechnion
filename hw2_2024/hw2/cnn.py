...
class CNN(nn.Module):
    ...
    def _make_feature_extractor(self):
        ...
        # ====== YOUR CODE: ======
        input_output_channels = [in_channels] + self.channels
        for i in range(len(input_output_channels) - 1):
            layers.append(
                nn.Conv2d(
                    input_output_channels[i],
                    input_output_channels[i + 1],
                    kernel_size=self.conv_params.get('kernel_size', 3),
                    stride=self.conv_params.get('stride', 1),
                    padding=self.conv_params.get('padding', 1),
                    bias=True,
                )
            )
            layers.append(ACTIVATIONS[self.activation_type](**self.activation_params))
            if (i + 1) % self.pool_every == 0:
                # Check if the input size is large enough for pooling
    ...
    def _make_mlp(self):
        ...
        mlp = MLP(self._n_features(), hidden_dims, nonlins + [nn.Identity()])
        # ========================
        return mlp
    ...
    def forward(self, x: Tensor):
        ...
        out = self.mlp(extracted_features.view(extracted_features.size(0), -1))
        # ========================
        return out
...
class ResidualBlock(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # ====== YOUR CODE: ======
        main_path = []
        channel_collection = [in_channels] + channels
        for i in range(len(channel_collection)-1):
            main_path.append(nn.Conv2d(
                channel_collection[i],
                channel_collection[i+1],
                kernel_size=kernel_sizes[i],
                bias=True,
                padding=(kernel_sizes[i]-1)//2
                )
            )
            if dropout > 0 and i < len(channel_collection)-2:
                main_path.append(nn.Dropout2d(dropout))
            if batchnorm and i < len(channel_collection)-2:
                main_path.append(nn.BatchNorm2d(channel_collection[i+1]))
            if i < len(channel_collection)-2:
                main_path.append(ACTIVATIONS[activation_type](**activation_params))
        self.main_path = nn.Sequential(*main_path)
        
        temp_path = [ACTIVATIONS[None]()]
        if channel_collection[0] != channel_collection[-1]:
            temp_path.append(
                nn.Conv2d(
                    channel_collection[0],
                    channel_collection[-1],
                    kernel_size=1,
                    bias=False
                )
            )
        self.shortcut_path = nn.Sequential(*temp_path)
        # ========================
    ...
    def forward(self, x: Tensor):
        ...
        out: Tensor = None
        # ====== YOUR CODE: ======
        resid_p = self.shortcut_path(x)
        main_p = self.main_path(x)
        out = resid_p + main_p
        # ========================
        out = torch.relu(out)
        return out
...
class ResNet(CNN):
    ...
    def _make_feature_extractor(self):
        ...
        # ====== YOUR CODE: ======
        block_counter = 0
        while block_counter < len(self.channels):
            block_counter_end = min(len(self.channels),block_counter + self.pool_every)
            block_channels = self.channels[block_counter:block_counter_end]
            block_kernels = [3] * len(block_channels)

            # Check if we should use a bottleneck block
            use_bottleneck = (
                self.bottleneck and in_channels == block_channels[0]
            )

            if use_bottleneck:
                block = ResidualBottleneckBlock(
                    in_channels, block_channels[1:-1], block_kernels[1:-1],
                ...
                block = ResidualBlock(
                    in_channels, block_channels, block_kernels,
                    batchnorm=self.batchnorm, dropout=self.dropout,
                    activation_type=self.activation_type,
                    activation_params=self.activation_params
                )
            ...
            # Check if we need to add a pooling layer
            if (block_counter_end - block_counter) == self.pool_every:
                layers.append(POOLINGS[self.pooling_type](**self.pooling_params))

            block_counter += self.pool_every
            in_channels = block_channels[-1]
            

        # Check if there are remaining layers that do not form a complete block
        if block_counter < len(self.channels):
            remaining_channels = self.channels[block_counter:]
            remaining_kernels = [3] * len(remaining_channels)
            block = ResidualBlock(
                in_channels, remaining_channels, remaining_kernels,
                batchnorm=self.batchnorm, dropout=self.dropout,
                activation_type=self.activation_type,
                activation_params=self.activation_params
            )
