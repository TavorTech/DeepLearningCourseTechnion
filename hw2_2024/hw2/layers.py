...
class LeakyReLU(Layer):
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        dx = dout * torch.where(x > 0, 1.0, self.alpha)
        # ========================
...
class ReLU(LeakyReLU):
    ...
    def __init__(self):
        # ====== YOUR CODE: ======
        # Set alpha to 0 for ReLU behavior
...
class Linear(Layer):
    ...
    def forward(self, x, **kw):
        ...
        out = x @ self.w.T + self.b  # y = x * W^T + b
        # ========================
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        # Compute gradients
        dx = dout @ self.w  # Gradient with respect to input
        self.dw += dout.T @ x  # Accumulate gradient for weights
        self.db += dout.sum(dim=0)  # Accumulate gradient for biases
        # ========================
...
class CrossEntropyLoss(Layer):
    ...
    def forward(self, x, y):
        ...
        log_softmax = x - torch.log(torch.sum(torch.exp(x), dim=1, keepdim=True))
        
        # Select the log probabilities of the correct classes
        true_class_log_probs = log_softmax[torch.arange(N), y]
        
        # Compute the mean negative log likelihood loss
    ...
    def backward(self, dout=1.0):
        ...
        dx /= N
        dx *= dout  # Apply chain rule with dout
        # ========================
...
class Dropout(Layer):
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        if self.training_mode:  # Only apply the mask during training
            dx = dout * self.mask / (1 - self.p)
        else:
            dx = dout  # Pass the gradients as is during testing
        # ========================
...
class Sequential(Layer):
    ...
    def forward(self, x, **kw):
        ...
        out = x  # Initialize with the input tensor
    
        for layer in self.layers:
            out = layer(out, **kw)  # Pass output of one layer to the next
        # ========================
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        din = dout  # Initialize with the gradient of the output
    ...
    def params(self):
        ...
        # ====== YOUR CODE: ======
        for layer in self.layers:
            params.extend(layer.params())  # Collect parameters from all layers
        # ========================
...
class MLP(Layer):
    ...
    def __init__(
        ...
        **kw,
        ...
        # ====== YOUR CODE: ======
        # Add fully connected (FC) layers with activations and optional dropout
        all_features = [in_features] + list(hidden_features) + [num_classes]
        for i in range(len(all_features) - 1):
            layers.append(Linear(all_features[i], all_features[i + 1]))
            if i < len(all_features) - 2:  # No activation after the last layer
                if activation == "relu":
                    layers.append(ReLU())
