...
class ClassifierTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        fw_pas = self.model.forward(X)
        loss = self.loss_fn(fw_pas,y)

        batch_loss = loss.item()
        num_correct = torch.sum((y==torch.argmax(fw_pas,dim=1)).int()).item()
...
class LayerTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        self.model.train()  # set training mode if needed
        y_pred = self.model(X)              # calls MLP.forward -> your custom layers
        
        # 3) Compute the loss
        loss = self.loss_fn(y_pred, y)      # e.g. CrossEntropyLoss(...) -> scalar
        
        # 4) Backward pass (manual)
        self.optimizer.zero_grad()          # zero out self.dw, self.db in each layer
        dloss = self.loss_fn.backward()     # partial derivative of loss w.r.t y_pred
        self.model.backward(dloss)          # backprop through your custom layers
        
        # 5) Update parameters
        self.optimizer.step()               # update w/b with your custom VanillaSGD
        
        # 6) Compute classification accuracy for the batch
        y_pred_labels = torch.argmax(y_pred, dim=1)
        num_correct = (y_pred_labels == y).sum().item()
        # ========================
    ...
    def test_batch(self, batch) -> BatchResult:
        ...
        y_pred = self.model(X)  # Forward pass through the custom model
    
        # Compute the loss
        loss = self.loss_fn(y_pred, y)  # Calculate the loss using the custom loss function
    
        # Compute the number of correct predictions
        y_pred_labels = torch.argmax(y_pred, dim=1)  # Predicted class labels
        num_correct = (y_pred_labels == y).sum().item()  # Count correct predictions as an integer
        # ========================
