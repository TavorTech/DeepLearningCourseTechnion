...
def mlp_experiment(
    ...
    params = {'lr': 0.01,'weight_decay': 0.001,'betas': (0.9, 0.99)}
    model = BinaryClassifier(model=MLP(in_dim=2,dims=[*[width]*depth,2],nonlins=[*['tanh']*depth,'softmax']))
    ...
    x, y = dl_utils.flatten(dl_valid)
    thresh = select_roc_thresh(model, x, y, plot=False)
    ...
    **kw,
    ...
    if model_type == "cnn" :
        model = model_cls(
            ...
            out_classes=10,
            channels=[filter for _ in range(layers_per_block) for filter in filters_per_layer],
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=lr, weight_decay=reg, amsgrad=amsgrad)
